...
struct TranscriptResult {
    ...
    end_time: f32,
    is_eos: Option<bool>,
    alternatives: Option<Vec<TranscriptAlternative>>,
}
...
impl Transcriber {
    ...
    async fn sync_and_send(
...

        {
            ...
            if let Some(ref buffer) = buffer {
                let running_time = state.in_segment.to_running_time(buffer.pts().unwrap());
    ...
    fn ensure_connection(&self) -> Result<(), gst::ErrorMessage> {
        ...
        let (ws, _) = {
            let _enter = RUNTIME.enter();
            futures::executor::block_on(connect_async(request)).map_err(|err| {
        ...
        let start_message = StartRecognition {
            ...
            transcription_config: TranscriptionConfig {
                language: match &settings.language_code {
        ...
        {
            let _enter = RUNTIME.enter();
            futures::executor::block_on(ws_sink.send(Message::Text(message))).map_err(|err| {
                gst::error!(
                    CAT,
                    imp = self,
                    "Failed to send RecognitionStarted: {}",
                    err
                );
                gst::error_msg!(
                    gst::CoreError::Failed,
                    ["Failed to send RecognitionStarted: {}", err]
                )
            })?
        }
        ...
        loop {
            let res = {
                let _enter = RUNTIME.enter();
                futures::executor::block_on(ws_stream.next())
                    .ok_or_else(|| {
                    ...
                    .map_err(|err| {
                        gst::error!(
                            CAT,
                            imp = self,
                            "Failed to receive RecognitionStarted: {}",
                            err
                        );
                        gst::error_msg!(
                            gst::CoreError::Failed,
...
impl ObjectSubclass for Transcriber {
    const NAME: &'static str = "RsSpeechmaticsTranscriber";
        ...
        match pspec.name() {
            ...
            "additional-vocabulary" => {
                ...
                for vocable in vocables.as_slice() {
                    ...
                    let content = match s.get::<String>("word") {
                        Ok(word) => word,
                        Err(_) => {
                            gst::warning!(
                                CAT,
                                imp = self,
                                "skipping vocable: {}, expected word field",
                                s
                            );
                            continue;
                        }
                    ...
                    let sounds_like: Option<Vec<String>> = match s.get::<gst::Array>("sounds_like")
                    {
                        Ok(sounds_like) => Some(
                            sounds_like
                                .as_slice()
        ...
        match pspec.name() {
            ...
            "additional-vocabulary" => {
                ...
                for vocable in &state.additional_vocabulary {
                    ...
                    if let Some(ref words) = vocable.sounds_like {
                        s.set(
                            "sounds_like",
                            gst::Array::new(words.iter().map(|word| word.to_send_value())),
                        );
                    }
