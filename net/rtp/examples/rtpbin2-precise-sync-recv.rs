...
impl Clock {
    ...
    // Builds a `Clock` with the specified RFC7273 description targetting the specifid SSRC
...
fn prepare_expected_medias(
    ...
    for media in &sdp.medias {
        ...
        // handle clock attributes, if any
        let media_level_clk =
        ...
        // and a source-level clock will be added to the pt-map using `ssrc-{ssrc}-ts-refclk`,
...
fn on_rtp_stream_added(recv: &gst::Element, pad: &gst::Pad, disable_visualizer: bool) {
    ...
    let stream_renderer_bin = gst::parse::bin_from_description_with_name(
        &format!(
            ...
            {optional_visu_branch}
            ",
            optional_visu_branch = if disable_visualizer {
                "".to_string()
            } else {
                format!(
                    "
            ...
            ",
                    ref_ts_caps = match ReferenceClock::from_str(refclk) {
    ...
    pad.add_probe(gst::PadProbeType::BUFFER, {
        |pad, info| {
            ...
            if let Some(meta) = buffer.meta::<gst::ReferenceTimestampMeta>() {
                ...
                pad.remove_probe(info.id.take().expect("executed once"));
            }
...
fn main() -> anyhow::Result<()> {
    ...
    let pipeline = initialise_pipeline(args, &loop_, sdp).context("initialising pipeline")?;
