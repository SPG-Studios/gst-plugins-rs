        ...
        for media_rtpmap in media.attributes_typed::<RtpMap>().flatten() {
            if media_rtpmap.payload_type == args.pt {
                if media_rtpmap != rtpmap {
                    bail!(
                        "Found incompatible existin media rtpmap {media_rtpmap:?} for payload type {} => select a different pt for {rtpmap}",
                        args.pt,
                    );
                }
...
fn initialise_pipeline(
    ...
    let raw_caps = gst::Caps::builder("audio/x-raw")
        ...
        .build();
    let audio_src = gst::parse::bin_from_description_with_name(
        &format!(
            "
    ...
    audio_src
        ...
        .add_probe(gst::PadProbeType::EVENT_DOWNSTREAM, move |pad, info| {
            ...
            let offset = bar_period - base_time % bar_period;
            eprintln!(
                "Aligning bar periods with clock time multiples: offset {offset}, base time {base_time}"
            );
    ...
    let payloader = match args.encoding_name.as_str() {
        "L8" => "rtpL8pay2",
        "L16" => "rtpL16pay2",
        "L24" => "rtpL24pay2",
        other => bail!("unsupported encoding name: {other}"),
    };
    let pt = args.pt;
    let audio_payloader = gst::parse::bin_from_description_with_name(
        &format!(
            "
            audioconvert name=audio-convert
            ! clocksync name=audio-sync
            ! {payloader} name=audio-payloader pt={pt}
    ...
    pipeline.add(&audio_payloader).unwrap();
    audio_src.link_pads(None, &audio_payloader, None).unwrap();
    ...
    audio_payloader
        ...
        .static_pad("src")
        .unwrap()
        .connect_caps_notify(move |pad| {
            let Some(payloader_and_caps) = Option::zip(pad.parent_element(), pad.allowed_caps())
            else {
                return;
            };
...
fn main() -> anyhow::Result<()> {
    ...
    let payloader = pipeline
        .by_name("audio-payloader")
        .expect("defined in initialise_pipeline");
