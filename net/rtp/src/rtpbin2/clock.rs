...
pub enum ClockError {
    #[error("Unsupported reference clock {}", .refclk)]
    Unsupported { refclk: ReferenceClock },

    #[error("Invalid reference clock {}", .0)]
    InvalidReferenceClock(sdp_types::AttributeError),
...
impl From<sdp_types::AttributeError> for ClockError {
    fn from(err: sdp_types::AttributeError) -> Self {
        ClockError::InvalidReferenceClock(err)
    }
}
...
pub struct SignalledClockInner {
    ...
    /// Offset to substract to an NTP time to convert to the target reference clock time
...
impl SignalledClockInner {
    ...
    fn try_new(refclk_str: &glib::GStr, gst_clock: gst::Clock) -> Result<Self, ClockError> {
        ...
        let refclk = match ReferenceClock::from_str(refclk_str) {
            Ok(refclk) => refclk,
            Err(err) => {
                return Err(err.into());
            }
        ...
        match &refclk {
            ReferenceClock::Local => {
                ...
                Ok(SignalledClockInner {
                    ...
                    ntp_offset: gst::ClockTime::from_seconds(UNIX_TO_NTP_TIME_OFFSET_SECONDS),
                })
            }
            ReferenceClock::Ntp(Ntp { server }) => {
                let system_time_offset = if gst_clock.type_() == gst::SystemClock::static_type() {
                    ...
                    gst::ClockTime::from_seconds(UNIX_TO_NTP_TIME_OFFSET_SECONDS)
            ...
            ReferenceClock::Ptp(Ptp { version, server }) => {
                let system_time_offset = if gst_clock.type_() == gst::SystemClock::static_type() {
                    ...
                    // user decided to use system clock to represent an PTP clock
                    // => adjust with leap seconds
                    gst::ClockTime::from_seconds(*UNIX_TO_PTP_TIME_OFFSET_SECONDS)
                ...
                let ntp_offset = gst::ClockTime::from_seconds(*NTP_TO_PTP_TIME_OFFSET_SECONDS);
            ...
            _ => Err(ClockError::Unsupported { refclk }),
        }
    }
...
pub fn get_media_clock_offset(media_clock_source: &MediaClockSource) -> u32 {
    // TODO handle other variants
    match &media_clock_source.clock {
        MediaClock::Direct(sdp_types::Direct {
            offset: Some(offset),
            rate,
        }) => {
            if rate.is_some_and(|r| !r.is_one()) {
                gst::fixme!(CAT, "unsupported non-unit rate: {media_clock_source}");
            }
            *offset
        }
        other => {
...
impl MediaLevelClock {
    fn add_mediaclk(&mut self, mediaclk: &str) -> anyhow::Result<()> {
        let mediaclk = MediaClockSource::from_str(mediaclk)?;
...
impl SourceLevelClock {
    ...
    fn get_reference_time_priv(
        ...
        // except that it returns the reference time relatively ti its actual epoch, not UTC (NTP time)

        let rtptime = rtptime as u64;

        // Get current time expressed relatively to the target reference clock epoch
        // (offset applies if it's a system clock targeting another reference clock)
        let now = now + self.clock.system_time_offset;

        // Current RTP time based on the estimated reference clock and the corresponding
        // RTP time period start
        let mut rtptime_period_start = now
            .nseconds()
            .mul_div_ceil(clock_rate as _, *gst::ClockTime::SECOND)
            .unwrap();
        ...
        gst::trace!(CAT, "{ssrc:#08x} ({ssrc}): cur reference time {now}");
        gst::trace!(
            CAT,
            "{ssrc:#08x} ({ssrc}): cur RTP time period start {sign}{} (RTP {sign}{rtptime_period_start})",
            gst::ClockTime::from_nseconds(
                rtptime_period_start
                    .mul_div_ceil(*gst::ClockTime::SECOND, clock_rate as _)
                    .unwrap()
            ),
        ...
        // litteraly any clock here for getting the correct result.
        if negative_rtptime_period_start {
            if rtptime >= rtptime_period_start {
                rtptime_ext = rtptime - rtptime_period_start;
            } else {
                // Packet is timestamped before the NTP clock epoch!
                rtptime_ext = 0;
            }
        } else {
            rtptime_ext = rtptime_period_start + rtptime;
        }
...
mod tests {
    ...
    const FIRST_JANUARY_2026_UNIX_TIME: gst::ClockTime = gst::ClockTime::from_seconds(
        ((2026 - 1970) * 365 + LEAP_YEARS_1970_TO_2026) * 24 * 60 * 60,
    );
    const FIRST_JANUARY_2026_PTP_TIME: gst::ClockTime = gst::ClockTime::from_seconds(
        ((2026 - 1970) * 365 + LEAP_YEARS_1970_TO_2026) * 24 * 60 * 60
    ...
    fn get_reference_time_system_clock() {
        ...
        let rtptime = (packet_sys_time
            .nseconds()
            .mul_div_ceil(CLOCK_RATE as u64, SECOND)
            .unwrap()
            & (u32::MAX as u64)) as u32;

        let ptp_clock_now = 6.seconds() + FIRST_JANUARY_2026_UNIX_TIME;
        ...
        let packet_ntp_time_ns =
            packet_sys_time.nseconds() + UNIX_TIME_TO_UTC_OFFSET_SECONDS * SECOND;
    ...
    fn get_reference_time_ntp_clock() {
        ...
        // build the object but to attempt to sync for this test
    ...
    fn get_reference_time_ptp_clock() {
        ...
        // * add UNIX time epoch
        // * add current leap seconds
        let packet_ntp_time_ns = packet_ptp_time.nseconds()
            + (UNIX_TIME_TO_UTC_OFFSET_SECONDS - *UTC_TO_TAI_LEAP_SECONDS) * SECOND;
        let ntp_time = NtpTime::from_duration(Duration::from_nanos(packet_ntp_time_ns));
        // reference time is expressed in PTP time
