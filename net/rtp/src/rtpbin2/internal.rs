...
impl SharedSessionInner {
    ...
    pub fn add_clock(
        ...
        gst::debug!(CAT, "adding clock: {refclk_str}, {clock:?}");
...
impl<'a> From<&'a rtp_types::RtpPacket<'a>> for RtpPacketHeader {
...
impl fmt::Display for RtpPacketHeader {
...
mod tests {
    ...
    fn media_and_src_level_clocks() {
        ...
        let media_level_meta_ref = gst::Caps::new_empty_simple("timestamp/x-ntp");
        ...
        // clock doesn't need to match the definition for this test
        shared_session
            .add_clock(glib::gstr!("local"), gst::SystemClock::obtain())
            .unwrap();
        shared_session
            .add_clock(
                glib::GString::from(format!("ntp={NTP_HOST}")).as_gstr(),
