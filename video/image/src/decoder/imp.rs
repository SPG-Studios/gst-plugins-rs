...
impl Decoder {
    ...
    fn render_single_frame<'a>(
        ...
        if let Some(caps) = self.srcpad.current_caps() {
        ...
        let mut outbuf = if allow_zerocopy {
            let mut b = gst::Buffer::from(Wrapper::Image(image));
            ...
            gst::Buffer::from(Wrapper::from(image))
        };
        {
                ...
                let duration = duration.map_or_else(
                    || {
                        fps.and_then(|v| {
                            ...
                            let nanos = i64::from(rest)
                                .mul_div_floor(1_000_000, i64::from(v.denom()))
                                .unwrap();
