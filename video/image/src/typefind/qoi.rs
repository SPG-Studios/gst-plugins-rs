...
fn type_find(typefind: &mut gst::TypeFind) {
    ...
    if let Ok(decoder) = image::codecs::qoi::QoiDecoder::new(cursor) {
        let d = decoder.dimensions();
        gst::log!(CAT, "extracted Qoi width and height: {}x{}", d.0, d.1);
        typefind.suggest(
            TypeFindProbability::Maximum,
            &Caps::builder("image/qoi").build(),
        );
    }
}

pub(crate) fn register(plugin: &gst::Plugin) -> Result<(), glib::BoolError> {
    let mediatypes = gst::Caps::builder("image/qoi").build();
