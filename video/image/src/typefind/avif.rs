...
fn type_find(typefind: &mut gst::TypeFind) {
    ...
    if let Ok(decoder) = image::codecs::avif::AvifDecoder::new(cursor) {
        let d = decoder.dimensions();
        gst::log!(
            CAT,
            "extracted AVIF surface width and height: {}x{}",
            d.0,
            d.1
        );
        typefind.suggest(
            TypeFindProbability::Maximum,
            &Caps::builder("image/avif").build(),
        );
    }
}

pub(crate) fn register(plugin: &gst::Plugin) -> Result<(), glib::BoolError> {
    let mediatypes = gst::Caps::builder("image/avif").build();
    gst::TypeFind::register(
        Some(plugin),
        "image/avif",
        // Needs to be bumped before video/quicktime
