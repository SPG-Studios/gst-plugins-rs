...
impl Decoder {
    ...
    fn render_many_frames(
        ...
        let (do_loop, mut repeat) = loop_info;

        while do_loop || repeat != 0 {
            for frame in frames {
                if self.srcpad.check_reconfigure() {
                    let color_info: Option<VideoColorimetry> =
                        ImageCicp(frame.buffer().color_space())
                            .try_into()
                            .inspect_err(|e| {
                                gst::warning!(
                                    CAT,
                    ...
                    let video_info =
                        gst_video::VideoInfo::builder(gst_video::VideoFormat::Rgba, wh.0, wh.1)
                    ...
                    allow_zerocopy = {
                        ...
                        let video_format_stride = video_info.stride();
                        let stride = frame.buffer().stride_in_bytes();
                        ...
                        n_planes == 1 && video_info.comp_stride(0) == stride
                    };
                    ...
                    if caps != prev_caps {
                        // FIXME: what happens if the push fails?
                ...
                // We can consume the frame here because AnimatedDecoder
                ...
                let mut out_buf = if allow_zerocopy {
                    ...
                    let mut b = gst::Buffer::from(Wrapper::Image(image.into()));
                    ...
                    gst::Buffer::from(Wrapper::from(DynamicImage::from(
                ...
                match self.srcpad.push(out_buf) {
                    Ok(_) => (),
                    Err(gst::FlowError::Flushing) | Err(gst::FlowError::Eos) => return Ok(()),
                    Err(error) => {
                        return Err(gst::error_msg!(
                            gst::StreamError::Failed,
                            ["Failed to push buffers: {error:?}"]
                        ));
                    }
                }
            }
    ...
    fn decode(&self) -> Result<(), gst::ErrorMessage> {
        ...
        let (wh, loop_count, frames) = match reader.format() {
            ...
            Some(ImageFormat::Png) => {
                ...
                (wh, apng_decoder.loop_count(), apng_decoder.into_frames())
            }
        ...
        let frames = frames.collect_frames().map_err(|v| {
            gst::error_msg!(gst::StreamError::Decode, ["Failed to get next frame: {v}"])
        })?;

        self.render_many_frames(&frames, wh, par, (do_loop, repeat))?;
        ...
        static PROPERTIES: LazyLock<Vec<glib::ParamSpec>> = LazyLock::new(|| {
            vec![
                ...
                glib::ParamSpecBoolean::builder("loop")
                    .nick("Loop")
                    .blurb("Respects the internal 'repeat' setting by default and overrides it to run infinitely if true")
                    .default_value(false)
                    .mutable_ready()
                    .build()
            ]
