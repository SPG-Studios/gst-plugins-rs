...
fn test_encoder() {
    ...
    for f in Format::all_encoder_formats() {
        for s in f.iter() {
            ...
            let formats = s.get::<gst::List>("format").unwrap();
            let container_format = Format::try_from(s)
                .map(|v| image::ImageFormat::try_from(v))
                .flatten()
                .unwrap();

            for f in formats.iter().map(|v| v.get::<&gst::glib::GStr>()) {
                let format = f.unwrap().as_str();
