...
- GstTensorMeta: hand keypoint tensors (and optional rotation tensors)
- GstAnalyticsRelationMeta: optional oriented hand bounding boxes

## Requirements

- Built with v1_30 feature support.
- keypointoverlay in the pipeline requires the dmorin keypointoverlay branch.
...
gst-launch-1.0 \
  ...
  ! keypointoverlay draw-labels=false keypoint-color=0xFF00FF00 keypoint-radius=4.0 \
  ! videoconvert \
  ! autovideosink
```
